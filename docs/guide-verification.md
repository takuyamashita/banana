...
| 11  | セットアップ手順・データベース             | `mise run gen` の次に `cargo run -p payroll-migrate`。`.sqlx/` をコミット                             | `DATABASE_URL` が設定されていると sqlx は `.sqlx/` より live DB を優先する。空の DB では infrastructure の `query!` がコンパイルエラーになり、migrate(bootstrap 経由で infrastructure に依存)自体がビルドできない(確認済み)                                        | mise の `[env]` に `SQLX_OFFLINE = "true"` を置き、キャッシュ更新は `SQLX_OFFLINE=false cargo sqlx prepare --workspace`(`mise run sqlx-prepare`)で行う。これで空の DB からでも `cargo run -p payroll-migrate` が通る(確認済み)                        |
...
- **動作確認の動画(`mise run e2e:video`)**: 台本(`e2e/videos/`)を Playwright で録画し、場面ごとに字幕を出して mp4 にする。台本も確かめるテストと同じく操作が通らなければ失敗するので、古い画面の動画が撮れ続けることはない。Playwright 付属の ffmpeg は webm しか作れないため、mise のタスクの `tools`(`conda:ffmpeg`)で入れる。このタスクを動かしたときだけ入り(初回約19秒)、`mise install` では入らないこと、普段の `mise run e2e` に台本が混ざらないこと、`-- -g 本人` で一部だけ撮れることを確かめた。
  - PR ごとに動画を撮る形にした。台本は PR ごとに `e2e/videos/<見せること>.spec.ts` に書き、`record()` で開いたブラウザを録画する。動画の名前は台本のファイル名とテスト名から付ける。`-- e2e/videos/walkthrough.spec.ts` でファイルを指定して撮れること、`-- --only-changed=origin/main` で変えた台本だけが撮れること、無い台本を指定すると止まることを確かめた。PR テンプレートに、撮り方と動画を貼る欄を置いた。`mise run pr:video` は、この PR の台本だけを撮って gh(mise で管理、2.101.0)の `--attach` で今のブランチの PR に貼る。初回は本文の末尾に印と見出しを付けて貼り、印があれば撮り直しとしてコメントに足す。PR がなければ撮る前に止まる。引数を記録するだけの偽の gh で、この3つの場合の呼び出しを確かめた。サンプルの PR(#1)で実際に貼り、本文の「動作確認」の節(テンプレートの説明の直後、署名の前)に `<video>` として出ることを確かめた。そこで分かったこと: gh は動画に alt を付けられない(`cannot set alt text on video`)。動画を貼る位置は、本文に `![](<パス>)` を1段落として書いておくと gh がその場で置き換える(書かなければ本文の末尾で、署名の後ろになる)。`--only-changed` は変えたファイルを import する台本も撮るので、`e2e/support/` や page object を変えた PR では、ほかの台本の動画も撮られる。マウスカーソルと押した場所も映す(録画には本物のカーソルが映らないので、要素を足す)。
  - PR の台本の置き場所を `e2e/videos/pr/<PR 番号>/` にした。`mise run pr:video` は、今のブランチの PR の番号のディレクトリの台本だけを撮る(`--only-changed` をやめた。共通のファイルを変えた PR で、ほかの台本まで撮られない)。ディレクトリの区切りまで指定するので、`pr/1/` は `pr/10/` に当たらない。台本がなければ撮る前に止まる。動画の出力先も `videos-out/pr/<PR 番号>/<台本>/` にした。サンプルの PR(#1)の台本を `e2e/videos/pr/1/` に移し、台本が古い場所のままだと止まること、撮り直しのコメントを確かめた。確認の後、サンプルの PR は閉じてブランチを消した(マージしていない)。
...
## 勤怠(timesheet)を別のサービスとして足した(2026-09-20)

サービスを分けたときに要るものを一通り示すため、派遣社員が月の稼働を申告し管理者が承認する「勤怠」を、給与とは別のプロセス・別のデータベースのサービスにした(ADR 0008)。

- **サービスを増やせる形にする(振る舞いは変えない)**: `backend/app` を `backend/services/payroll` にし、サービスの起動と停止・relay・キューの受け手・設定の読み込み・マイグレーションの流れを共通の crate(`platform-service`・`platform-messaging`・`platform-db`)に出した。業務の層の ports は共有しない。移した後に E2E(7件)・スモーク(26件)・Rust のテストが変わらず通ることを確かめた。
- **2つのサービスを同じ MySQL に載せると壊れる所**: relay のロック(`GET_LOCK`)は DB サーバー全体で共有されるので名前をサービスごとにした。ローカルの `.env` の `APP__DATABASE__URL` が両方のサービスに効いてしまうので、設定を上書きする環境変数の接頭辞をサービスごとにした(`PAYROLL__…`・`TIMESHEET__…`)。`query!` が見る DB は crate ごとの `sqlx.toml` で分けた(sqlx 0.9 の `sqlx-toml` 機能。sqlx-cli は `DATABASE_URL` がないと動かないので、給与の接続先を渡す)。
- **サービスごとのデータベースとユーザー**: ユーザーは自分のデータベースしか触れない。別のデータベースを読めないこと、データベース名に文を埋め込めないことをテストで確かめた。AWS では各サービスの migrate が管理者でデータベースを作る。
- **出来事の約束は proto**: `acme.payroll.events.v1`・`acme.timesheet.events.v1`。ペイロードは proto の JSON 形(int64 は文字列)で、Rust は neoeinstein-prost-serde で serde を生成する(v0.3 系は生成先のパスを見つけられず失敗し、v0.4.0 で動いた)。出来事を記録するようになる前の派遣社員・案件を知らせるマイグレーションは、手前の版の DB にデータを入れてから流し、Rust が送るのと同じ形になることを確かめた。
- **配り方**: AWS は SNS FIFO トピック → 受け手ごとの SQS(購読のフィルターで種類を絞る)。ローカルは relay が受け手のキューへ直接送る。受け手は server の中で自分のキューを読み、届いた内容で置き換える(2回届いても同じ)。受け手の処理をクロージャで渡すと常駐タスク(`tokio::spawn`)に載せられなかったので、`Handler` トレイトにした。
- **サービスをまたぐ流れ**: 給与で登録 → 勤怠に届く → 本人が申告 → 管理者が承認 → 給与に「承認された稼働」が届き、給与明細の明細行に入れられる。スモークテスト(サービスをまたぐ11件を足して37件)と E2E(承認の流れ・差し戻しの流れの2件を足して9件。4回繰り返して通過)で確かめた。Jaeger で、勤怠の承認のリクエストから給与の受け手までが1つのトレースになることを確かめた。
- **結果整合**: 給与で登録した派遣社員が勤怠に届く前に本人が開くと、「まだ勤怠に届いていません」と伝える(API テストあり)。
- **画面**: 派遣社員に「勤怠」、管理者に「勤怠の承認」、給与明細の作成に「承認済みの勤怠から明細を入れる」。画面は transport を1つだけ扱い、RPC のサービスで宛先を振り分ける(テストあり)。
- **2つのシステムを見分ける画面**: 1つの画面から給与と勤怠を使うので、画面の枠をシステムごとの名前(給与管理・勤怠管理)と色(給与は青、勤怠は橙)にし、メニューもシステムごとに分けてその色で並べた。各ルートが `staticData.system` でどちらかを決め、枠は `data-system` を付けるだけで、色は tokens.css の変数(`--color-accent`)の入れ替えで変わる。ログインした人の画面がどれもシステムを決めていることをテストで確かめ、決め忘れると落ちることも確かめた。`role="group"` は oxlint(jsx-a11y の prefer-tag-over-role)に断られたので、システムごとのメニューはそれぞれ `nav` にした。
- **2つのサービスを画面だけで通す E2E**(`e2e/tests/journeys`): 管理者が画面で案件と派遣社員を登録 → 本人が初回ログインして勤怠を申告 → 管理者が承認 → 承認済みの勤怠から給与明細を作って確定 → 本人が自分の給与明細を見る、を API を使わずに通す。見出しとその色がシステムごとに切り替わることも見る。サービスをまたぐ所は、開き直しながら届くまで待つ(固定の sleep は置かない)。勤怠の承認のテストは勤怠の中(承認済みになるまで)だけにした。10件を3回繰り返して通過。
- **AWS**: ALB とクラスターを共有し、RPC のパスで振り分ける。サービスごとに ECR・タスク・migrate・権限。validate・tflint・trivy が3環境で通る。deploy はサービスごとにイメージを作り、migrate を流し、全サービスの安定を待つ(actionlint 通過)。勤怠のイメージ(63MB)を作り、コンテナで起動と停止(受け手 → relay → server の順に止まり、終了コード 0)を確かめた。

## この環境では確かめていないこと

- AWS への `terraform apply` と実際のデプロイ(`deploy`・`deploy-environment` ワークフロー。plan・承認・apply の分割を含む)。SNS FIFO トピックから SQS FIFO キューへの配送と購読のフィルター、ALB のパスでの振り分け、RDS での各サービスの migrate(データベースとユーザーの作成)も未確認。Terraform は validate・tflint・trivy まで、ワークフローは actionlint と、使っている AWS CLI の引数検証・jq の変換まで。`infra/bootstrap` の apply、RDS の管理者シークレットでの migrate、CloudFront の CSP と CloudFront Function(画面の URL の置き換え)、アラームの通知も未確認。
...
| domain・usecase の単体テスト、DB 結合テスト(testcontainers)、API テスト(tonic クライアント)、DB ユーザーの権限                                                                                                                                                                                               | `mise run test:rust`                                                                                         | 145件通過                                                  |
| フロントの lint(oxlint・stylelint・CSS の層・依存の向き)・型・コンポーネントテスト・ビルド                                                                                                                                                                                                                   | `mise run lint:ts`・`lint:css`・`lint:deps`・`test:ts`・`pnpm --filter web build`                            | 通過(51件)                                                 |
| gRPC の主要シナリオ(認証・認可・二重確定・入力検証・金額計算・明細行の案件名)                                                                                                                                                                                                                                | `mise run smoke`                                                                                             | 37件通過                                                   |
| outbox → relay → ElasticMQ → consumer(再配信の冪等性を含む)                                                                                                                                                                                                                                                  | local_poller(振込依頼の記録まで)・ElasticMQ の DLQ(5回で移る)・`cargo lambda invoke`                         | 期待どおり                                                 |
| ブラウザの一連の流れ(Keycloak ログイン・ログアウト・初回パスワード変更・給与明細の作成と確定・本人だけが確定済みの明細を見られる・作成中は本人に見えない・セッション切れ・ログイン後の戻り先・ブラウザの戻る・勤怠の申告から承認・承認済みの勤怠からの給与明細・差し戻し・2つのサービスを画面だけで通す流れ) | `mise run e2e`(Playwright)                                                                                   | 10件通過                                                   |
