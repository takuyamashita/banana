...
/// 申告された勤務表と、それを書いた派遣社員
#[derive(Debug)]
pub struct SubmittedTimesheetView {
...
impl ListSubmittedTimesheetsUseCase {
    ...
    pub async fn execute(&self) -> Result<Vec<SubmittedTimesheetView>, UseCaseError> {
        let mut views = Vec::new();
        for timesheet in self.timesheets.list_submitted().await? {
            let staff = self.staff.find(timesheet.content().staff_id()).await?;
            views.push(SubmittedTimesheetView { timesheet, staff });
        }
        Ok(views)
    }
}
...
impl ApproveTimesheetUseCase {
    ...
    pub fn new(
        ...
        Self { timesheets, outbox, db, clock }
    }

    pub async fn execute(&self, id: TimesheetId) -> Result<Timesheet, UseCaseError> {
...
impl ReturnTimesheetUseCase {
    #[must_use]
    pub fn new(timesheets: Arc<dyn TimesheetRepository>, db: Arc<dyn Database>) -> Self {
        Self { timesheets, db }
    }
