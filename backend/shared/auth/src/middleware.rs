//! gRPC の認証の入口。どのサービスも同じミドルウェアでトークンを検証し、ハンドラはここの関数で利用者を取り出す

use std::sync::Arc;

use crate::{AuthError, OidcVerifier};
...
/// ログインしている利用者。いなければ Unauthenticated
pub fn current_user<T>(request: &tonic::Request<T>) -> Result<AuthenticatedUser, Status> {
    request
        .extensions()
...
/// 管理者だけに許す操作の入口
pub fn require_admin<T>(request: &tonic::Request<T>) -> Result<AuthenticatedUser, Status> {
