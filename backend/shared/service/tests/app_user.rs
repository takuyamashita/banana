//! アプリ用の DB ユーザーの権限とデータベースの用意を、使い捨ての MySQL で確かめる

// allow-unwrap-in-tests は #[test] 関数の中にしか効かず、補助関数は対象外
#![allow(clippy::unwrap_used)]

use platform_service::migrate::{Credentials, ensure_app_user, ensure_database};
...
async fn mysql() -> (ContainerAsync<Mysql>, u16, MySqlPool) {
    ...
    sqlx::query("create table projects (id bigint not null auto_increment primary key, name varchar(100) not null)")
        .execute(&admin)
        .await
        .unwrap();
...
#[tokio::test]
async fn a_service_user_cannot_read_another_services_database() {
    let (_container, port, admin) = mysql().await;
    // 1つの MySQL に2つのサービスのデータベースを置く
    ensure_database(&admin, "timesheet").await.unwrap();
    // 2回目は何もしない
    ensure_database(&admin, "timesheet").await.unwrap();
    sqlx::query("create table timesheet.timesheets (id bigint not null primary key)")
        .execute(&admin)
        .await
        .unwrap();
    ensure_app_user(&admin, "test", &app("AppPassword0123456789")).await.unwrap();
    ...
    let pool = connect_as(port, "AppPassword0123456789").await.unwrap();
    let err =
        sqlx::query("select count(*) from timesheet.timesheets").execute(&pool).await.unwrap_err();
    assert!(err.to_string().contains("command denied"), "{err}");
    assert!(ensure_database(&admin, "x`; drop database test; --").await.is_err());
}
